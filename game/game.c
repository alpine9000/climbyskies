...
void setupCopper(void)
{
  ...
  for (int y = 0; y < SCREEN_HEIGHT; y++) {
    ...
    for (int b = 0; b < SCREEN_BIT_DEPTH; b++) {
      copper.lines[y].bpl[b*4] = BPL6PTL;// + (b*4);
      copper.lines[y].bpl[(b*4)+2] = BPL6PTH;// + (b*4); + (b*4);
...
static void 
setCopper(int16_t line, frame_buffer_t ptr)
{
...
game_init()
{
  ...
  copper.sprites[0x11] = (uint32_t)&nullSprite >> 16;
  copper.sprites[0x13] = (uint32_t)&nullSprite;
  copper.sprites[0x15] = (uint32_t)&nullSprite >> 16;
  copper.sprites[0x17] = (uint32_t)&nullSprite;
...
scrollBackground()
{
  ...
  if (hscroll != 0) {
    setCopper(0, frameBuffer+((FRAME_BUFFER_HEIGHT-hscroll)*SCREEN_BIT_DEPTH*SCREEN_WIDTH_BYTES));
  }

  if (hscroll < SCREEN_HEIGHT) {
    setCopper(hscroll, frameBuffer);
    lastLine = hscroll;
  }
